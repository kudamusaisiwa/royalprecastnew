...
export default function Collections() {
  ...
  const handleStatusToggle = async (orderId: string) => {
    try {
      ...
      const newStatus = order.collectionStatus === 'completed' ? 'pending' : 'completed';
      await useOrderStore.getState().updateOrder(orderId, { collectionStatus: newStatus });

      setToastMessage(`Collection marked as ${newStatus}`);
  ...
  const weeklyCollections = useMemo(() => {
    ...
    return weekDays.map(day => {
      const dayCollections = orders.filter(order => {
        if (order.deliveryMethod !== 'collection' || !order.collectionDate) return false;
        
        const collectionDate = order.collectionDate.toDate?.() || new Date(order.collectionDate);
        
        // Check if the collection is on this day
        const isSameDay = 
          collectionDate.getFullYear() === day.getFullYear() &&
          collectionDate.getMonth() === day.getMonth() &&
          collectionDate.getDate() === day.getDate();
        
        return isSameDay && (filter === 'all' || order.collectionStatus === filter);
      }).sort((a, b) => {
        const dateA = a.collectionDate.toDate?.() || new Date(a.collectionDate);
        const dateB = b.collectionDate.toDate?.() || new Date(b.collectionDate);
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center space-x-4">
          ...
          >
            <option value="all" className="text-gray-900 dark:text-gray-200">All Collections</option>
      ...
      <div className="grid grid-cols-7 gap-4">
        {weeklyCollections.map(({ date, collections }, index) => (
                ...
                return (
                  ...
                  >
                    ...
                    <div className="text-xs text-gray-600 dark:text-gray-400 flex items-center">
                      ...
                      <span>
                        {collectionDate.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                      </span>
                    </div>
                  </div>
                );
